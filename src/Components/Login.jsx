import React from "react";
import { Link } from "react-router-dom";
import loginstyle from "./Login.module.css";


export default function Login() {
  return (
    <div className={loginstyle.login}>
<form >
      <input type="text" name="first_name" placeholder="Email Address" required />
      <br/>
      <br/>
      <input type="password" name="password"  placeholder="Password" required />
      <br/>
      <br/>
      <button  className={loginstyle.button_common} type="submit">LOGIN</button>
      <br/>
      <br/>
           <Link to="/forget-password"><p className={loginstyle.forget}>Forget password?</p></Link>
       <p>or login with</p>
     <div className={loginstyle.img}>
       <img src="https://1000logos.net/wp-content/uploads/2016/11/New-Google-Logo.jpg"/>
       <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZqTjy_WlpCpVb5abviBQSOL7CklFa4WKuuA&usqp=CAU"/>
       <img src="data:image/png;base64,"/>
     </div>
    <p>Don’t have an account? Create new now!</p>
    <p>By signing up, you are agree with our Terms & Conditions</p>

</form>
    </div>
  );
}
